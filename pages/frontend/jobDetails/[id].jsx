...
export default function JobDetails() {
    ...
    const { data, error , isLoading } = useSWR(
        id ? `/get-specified-job` : null,
        () => get_specified_job(id, locale)
    );
    ...
    const handleApply = () => {
        if (!user) return toast.error(t('jobDetails:login_first_error'));
    ...
    const handleBookMark = async () =>  {
        ...
        const res = await book_mark_job(data, locale);
        if(res.success) {
                    ...
                    <>
                        ...
                        <div className='w-full  py-20 flex items-center md:px-8 px-2  justify-center flex-col  '>
                            <div className='w-full h-40 bg-gray-50 text-indigo-600 font-bold flex items-center justify-center flex-col'>
                                <h1 className='text-3xl'>{t('jobDetails:title')}</h1>
                            </div>
                                ...
                                <div className='flex w-full px-8 md:px-20 items-start md:flex-row flex-col md:justify-between justify-center'>
                                    ...
                                    <div className='md:px-4 mb-1 px-2 md:mx-2 flex flex-col items-start justify-center'>
                                        <div className='flex items-center justify-center mb-1'>
                                            ...
                                            <p className='font-semibold text-base mx-1'>{t('jobDetails:job_poster')}</p>
                                            <p className=' text-sm text-gray-800 mx-1'>{JobDetails?.user?.name}</p>
                                        </div>
                                        <div className='flex items-center justify-center mb-1'>
                                            <MdEmail className='text-xs font-semibold text-indigo-600' />
                                            <p className='font-semibold text-base mx-1'>{t('jobDetails:email')}</p>
                                    ...
                                    <div className='md:px-4 mb-1 px-2 md:mx-2 flex flex-col items-start justify-center'>
                                        <div className='flex items-center justify-center mb-1'>
                                            ...
                                            <p className='font-semibold text-base mx-1'>{t('jobDetails:location')}</p>
                                            <p className=' text-sm text-gray-800 mx-1'>Rawalipindi</p>
                                        </div>
                                        <div className='flex items-center justify-center mb-1'>
                                            <MdCategory className='text-xs font-semibold text-indigo-600' />
                                            <p className='font-semibold text-base mx-1'>{t('jobDetails:category')} </p>
                                    ...
                                    <div className='md:px-4 mb-1 px-2 md:mx-2 flex flex-col items-start justify-center'>
                                        <div className='flex items-center justify-center mb-1'>
                                            ...
                                            <p className='font-semibold text-base mx-1'>{t('jobDetails:job_type')} </p>
                                        ...
                                        <div className='flex items-center justify-center mb-1'>
                                            ...
                                            <p className='font-semibold text-base mx-1'>{t('jobDetails:salary')} </p>
                                    ...
                                    <div className='flex items-center justify-center'>
                                        {
                                            JobDetails?.user?.email === user?.email ? (
                                                <p className='text-xs text-red-500'>{t('jobDetails:unable_to_apply')}</p>
                                            ) : (
                                                <div className='flex items-center justify-center  '>
                                                    <BsFillBookmarkCheckFill onClick={handleBookMark} className='text-indigo-600 text-4xl cursor-pointer  mx-2'/>
                                                    <button onClick={handleApply} className='md:px-6 md:py-3 px-3 py-2 mt-2 md:mt-0 bg-indigo-500 rounded text-base tracking-widest uppercase transition-all duration-700 hover:bg-indigo-900 text-white  '>{t('jobDetails:apply_position')}</button>
                                                </div>
                                            )
                                        }
                                    </div>
                            ...
                            <div className='w-full md:px-4 py-2 flex items-center md:items-start md:flex-row flex-col justify-start md:justify-center'>
                                <div className='md:w-8/12 w-full md:px-4 py-8 flex flex-col items-center content-start justify-center '>
                                    <h1 className='text-center lg:text-2xl font-semibold text-xl mb-4 uppercase border-b-2 border-indigo-600 py-2'>{t('jobDetails:job_description')}</h1>
                                    <p className='px-4'>{JobDetails?.description}</p>
                                </div>
                                <div className='md:w-4/12 w-full py-8 px-4 md:px-10'>
                                    <h1 className=' text-2xl font-semibold mb-2'>{t('jobDetails:job_summary')}</h1>
                                    <div className='flex items-center justify-start mb-3'>
                                        <RiUserSearchFill className='text-base font-semibold text-indigo-600' />
                                        <p className='font-semibold text-base mx-1'>{t('jobDetails:total_vacancies')}</p>
                                    ...
                                    <div className='flex items-center justify-start mb-3'>
                                        ...
                                        <p className='font-semibold text-base mx-1'>{t('jobDetails:deadline')}</p>
                                    ...
                                    <div className='flex items-center justify-start mb-3'>
                                        ...
                                        <p className='font-semibold text-base mx-1'>{t('jobDetails:experience_required')}</p>
                                        <p className=' text-sm text-gray-800 mx-1'>{JobDetails?.job_experience}</p>
                                    </div>
                                </div>
                            </div>
                            <div className='w-full px-2 md:px-8 mb-2 flex flex-col'>
                                <h1 className='text-xl font-semibold lg:text-2xl '>{t('jobDetails:related_jobs')}</h1>
                                        ...
                                        machingData?.length === 0 ? (
                                            <>
                                                <div className='md:w-96 w-full py-3 mx-4 my-2 flex items-center md:items-start px-6 justify-start md:justify-center flex-col rounded bg-gray-50'>
                                                    <p  className='text-xs font-semibold text-red-600 uppercase'>{t('jobDetails:no_similar_jobs')}</p>
                                                </div>
                                                ...
                                                <div key={item?._id} className='md:w-96 w-full py-3 mx-4 my-2 flex items-center md:items-start px-6 justify-start md:justify-center flex-col rounded bg-gray-50'>
                                                    ...
                                                    <div className='flex flex-col px-1 md:px-4 py-6 items-start justify-center'>
                                                        <div className='flex px-1 md:px-4 items-center justify-start mb-2'>
                                                            ...
                                                            <p className='font-semibold text-xs md:text-base mx-1'>{t('jobDetails:job_type')} </p>
...
export async function getStaticProps({ locale }) {
    return {
        props: {
            ...(await serverSideTranslations(locale, ['common', 'navbar', 'jobDetails'])),
        },
    };
}
