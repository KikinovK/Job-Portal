...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    e.preventDefault();
    if (!formData.email) {
      setError({ ...error, email: t('auth:login_email_error') })
      return;
    }
    if (!formData.password) {
      setError({ ...error, password: t('auth:login_pass_error') })
      return;
    }

    const res = await login_me(formData, locale);
    if(res.success)
    {
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-start">
                  ...
                  <div className="ml-3 text-sm">
                    <label htmlFor="remember" className="text-gray-500 dark:text-gray-300">{t('auth:remember_label')}</label>
                  </div>
                </div>
                <Link href="/auth/forget-password" className="text-sm font-medium text-indigo-600 hover:underline ">{t('auth:forget_pass_link')}</Link>
              </div>
              <button type="submit" className="w-full text-white bg-indigo-600 hover:bg-indigo-700 focus:ring-4 focus:outline-none focus:ring-indigo-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-indigo-600 dark:hover:bg-indigo-700 dark:focus:ring-indigo-800">{t('auth:login_button')}</button>
              <p className="text-sm font-light ">
                {t('auth:no_account')}<Link href="/auth/register" className="font-medium text-indigo-600 hover:underline ">{t('auth:register_link')}</Link>
              </p>
            </form>
          </div>
        </div>
